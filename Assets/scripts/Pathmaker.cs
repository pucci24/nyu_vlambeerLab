...
//	DECLARE CLASS MEMBER VARIABLES:
	int counter=0; 									// counter var will track how many floor tiles I've instantiated
	public int pathTilespan=40, mapSize=500;
	public float turnRRange=0.15f;
	public static int globalCounter=0;
	public Transform floorPrefab; 					//assign the prefab in inspector;
	public Transform pathmakerSpherePrefab; 		//assign the prefab in inspector;
	// you'll have to make a "pathmakerSphere" prefab later
	void Start () {
		mapSize = Random.Range (500, 700);
		pathTilespan = (Random.Range (20, 50));
		turnRRange = Random.Range (0.1f,0.25f);
	}

	void Update () {
		if (globalCounter < mapSize) {
			if (counter < pathTilespan) {//		If counter is less than 50, then:
				float randNum = Random.Range (0.0f, 1.0f);//		Generate a random number from 0.0f to 1.0f;
				if (randNum < turnRRange) {
					transform.Rotate (0f, 90f, 0f);
				}//		If random number is less than 0.25f, then rotate myself 90 degrees;
			else if (randNum > 0.35f && randNum < 0.5f) {
					transform.Rotate (0f, -90f, 0f);
				}//			... Else if number is 0.25f-0.5f, then rotate myself -90 degrees;
			else if (randNum > 0.96f && randNum < 1.0f) {
...
//		// end elseIf
				...
				transform.position += transform.forward * 5f;	//		Move forward ("forward" in local space, relative to the direction I'm facing) by 5 units;
